ONPREMISES_WORKERS = IO.readlines(File.join(__dir__, '../workers_prd.txt'), chomp: true)

{
  ...
  core_services: {
    ...
    Worker: ONPREMISES_WORKERS.map {|ip| [ip, 26000] },
    ResourceService: ONPREMISES_WORKERS.map {|ip| [ip, 28000] },
  },
  other_services: {
  ...
  database: "postgresql+psycopg2://cmsuser:#{@secrets.fetch(:db_cmsuser_password)}@cms-prd-db.c9ge2hh8rox6.ap-northeast-1.rds.amazonaws.com/cmsdb",
  ...
  redis_url: "redis://ioicms-prd.vozztv.ng.0001.apne1.cache.amazonaws.com:6379/0",
  ...
  # "you trust.",
  rankings: ["http://citrus:aikotoba@cms-prd-ranking-001.apne1.aws.ioi18.net:80/"],
  https_certfile: nil,
  ...
  s3_backend_bucket: 'ioi18-cms-files-prd',
  s3_backend_prefix: '',

  # cms::clientlb cookbook
  s3_backend_proxy: 'localhost:18080',
  s3_backend_fetch_base_url: 'http://localhost:10080/ioi18-cms-files-prd/',
