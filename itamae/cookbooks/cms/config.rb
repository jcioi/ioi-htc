node.reverse_merge!(
  cms: {
    config: {
      ...
      database: "postgresql+psycopg2://cmsuser:sorah-cms@sorah-cms-101.cwsmfajr96lt.ap-northeast-1.rds.amazonaws.com/cmsdb",
      ...
      secret_key: node[:secrets].fetch(:cms_secret_key),
...
file '/etc/cms.conf' do
  content "#{node[:cms].fetch(:config).to_json}\n"
  owner 'root'
  group 'cmsuser'
  mode  '0640'
end
